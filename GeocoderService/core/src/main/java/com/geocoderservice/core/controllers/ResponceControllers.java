...
public class ResponceController {
    ...
    @PostMapping
    public ResponseEntity<Responce> save(@RequestBody Responce responce) {
        return service.save(responce).map(u -> new ResponseEntity<>(u, HttpStatus.OK))
                .orElseThrow(() -> new ResponceException(
